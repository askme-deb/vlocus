        ...
        <div class="ms-auto">
            @can('Branch Create')
                <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <a class="btn btn-primary px-4" href="{{ route('branch.create') }}"><i
                        ...
                        <tbody>
                            ...
                            @foreach ($branchs as $user)
                                <tr>
                                    ...
                                    @unless(auth()->user()->hasRole('Company'))
                                    <td>{{ $user->branch?->company?->name ?? 'N/A' }}</td>
                                    @endunless
                                    @canany(['Branch Edit', 'Branch Delete'])
                                    <td>
                                        ...
                                        @can('Branch Edit')
                                            <a href="{{ route('branch.edit', $user->id) }}" class="btn btn-icon btn-sm"
                                                title="Edit"><i class="text-primary" data-feather="edit"></i></a>
                                        @endcan
                                        @can('Branch Delete')
                                            <a class="btn" href="javascript:void(0);" onclick="deleteItem(this)"
                                                    data-url="{{ route('branch.delete', $user->id) }}" data-item="Driver"
                                                    alt="delete"><i
