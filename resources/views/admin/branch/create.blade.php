...
@section('content')
        ...
        <div class="ms-auto">
            @can('Branch Create')
                <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <a class="btn btn-primary px-4" href="{{ route('branch.index') }}"><i
    ...
    <form class="needs-validation" action="{{ route('branch.store') }}" method="post" novalidate enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="col-md-9">
                <div class="card mt-4">
                            ...
                            <div class="col-md-9">
                                <select class="form-control input-height" name="vehicle_id" required>
                                    ...
                                    @foreach ($vehicles as $item)
                                        <option value="{{ $item->id }}"
                                            @if (old('vehicle_id') == $item->id) selected @endif>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
