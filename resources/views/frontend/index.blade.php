                                ...
                                @foreach ($vehicle_types as $index => $vehicle_type)
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link {{ $index == 0 ? 'active' : '' }}"
                                            id="tab-{{ $vehicle_type->id }}" data-bs-toggle="tab"
                                            data-bs-target="#tab-pane-{{ $vehicle_type->id }}" type="button" role="tab"
                                            aria-controls="tab-pane-{{ $vehicle_type->id }}"
                                            aria-selected="{{ $index == 0 ? 'true' : 'false' }}">
                                            <img src="{{ $vehicle_type->getFirstMediaUrl('vehicle-type-icon') }}"
                                                alt=""
                                                style="display: {{ is_have_image($vehicle_type->getFirstMediaUrl('vehicle-type-icon')) }};">
                                        </button>
                                    </li>
                                @endforeach
                            </ul>
                            ...
                            <div class="tab-content" id="myTabContent">
                                @foreach ($vehicle_types as $index => $vehicle_type)
                                    <div class="tab-pane fade {{ $index == 0 ? 'show active' : '' }}"
                                        id="tab-pane-{{ $vehicle_type->id }}" role="tabpanel"
                                        aria-labelledby="tab-{{ $vehicle_type->id }}" tabindex="0">

                                        <form action="{{ route('book-buss') }}" method="GET" class="ajax-booking-form"
                                            ...
                                            <div class="align-items-center">
                                                ...
                                                <div class="col-lg-12 col-md-12 d-flex mb-2 position-relative">
                                                    <i class="fa fa-circle position-absolute"
                                                        style="left: 10px; top: 50%; transform: translateY(-50%); color: #555;"></i>
                                                    <input type="text" name="from" id="from" class="from" value="{{ $from ?? '' }}"
                                                ...
                                                <div class="col-lg-12 col-md-12 d-flex mb-2 position-relative">
                                                    <i class="fa fa-stop-circle position-absolute"
                                                        style="left: 10px; top: 50%; transform: translateY(-50%); color: #555;"></i>
                                                    <input type="text" name="to" id="to" class="to" value="{{ $to ?? '' }}"
                                                ...
                                                <div class="d-flex col-lg-12 col-md-12 col-12 col-sm-12">
                                                    <div class="col-lg-8 col-md-8 col-8 col-sm-8 d-flex flex-column">
                                                        <input class="flatpickr-input" type="text" name="date"
                                                            placeholder="Select Date" value="{{ $date ?? date('Y-m-d') }}"
                                                    ...
                                                    <div class="col-lg-4 col-md-4 col-4 col-sm-4">
                                                        <button
                                                            @auth type="submit" @else type="button"
                                                            data-bs-toggle="modal" data-bs-target="#exampleModalCenter" @endauth
...
<script>
    ...
    // });
          ...
          $(document).on('submit', '.ajax-booking-form', function(e) {
            e.preventDefault();
            $.ajax({
                url: "{{ route('vehicles.direct-search') }}",
                method: "GET",
                data: $(this).serialize(),
                beforeSend: function() {
                    $('.viewBusesBox .container').html('<p>Loading...</p>');
